...
namespace chapter4 {
    ...
    struct DepthPrePass : public FrameGraphRenderPass {
        void declare_frame_graph_node( FrameGraphResourceContext& context ) override {
            ...
            FrameGraphNodeCreation_v2 depth_prepass_node_info {
                .outputs = {
            ...
            context.frame_graph->add_node_v2( depth_prepass_node_info );
        }
        ...
        void update_psos( FrameGraphResourceContext& context, PipelineUpdatePhase phase ) override {
            ...
            chapter4::pipeline_cache.insert( hash_calculate( "depth_prepass" ), depth_prepass_pipeline );
        }
        ...
        void render( FrameGraphRenderContext& context ) override {
            ...
            for ( RenderItem& render_mesh : render_view->opaque_items ) {
                ...
                gpu_commands->bind_descriptor_set( { renderer->gpu->bindless_descriptor_set, chapter4::scene_ds }, { chapter4::scene_cb_offset } );
    ...
    struct GBufferPass : public FrameGraphRenderPass {
        ...
        void declare_frame_graph_node( FrameGraphResourceContext& context ) override {
            ...
            FrameGraphNodeCreation_v2 gbuffer_node_info {
                .inputs = {
                    {
                        .type = FrameGraphResourceType_Attachment,
            ...
            context.frame_graph->add_node_v2( gbuffer_node_info );
        }
        ...
        void update_psos( FrameGraphResourceContext& context, PipelineUpdatePhase phase ) override {
                ...
                .stages = {
                    {
                        ...
                        .headers = {
                            ...
                            "glsl/chapter4/scene_mesh_data.h"
                        },
                        .type = VK_SHADER_STAGE_VERTEX_BIT,
                    },
                    {
            ...
            chapter4::pipeline_cache.insert( hash_calculate( "gbuffer_no_cull" ), gbuffer_no_cull_pipeline );
            ...
            chapter4::pipeline_cache.insert( hash_calculate( "gbuffer_cull" ), gbuffer_cull_pipeline );
        }
    ...
    struct LighPass : public FrameGraphRenderPass {
        void declare_frame_graph_node( FrameGraphResourceContext& context ) override {
            ...
            FrameGraphNodeCreation_v2 lighting_node_info {
                .inputs = {
                    {
            ...
            context.frame_graph->add_node_v2( lighting_node_info );
        }
        ...
        void update_psos( FrameGraphResourceContext& context, PipelineUpdatePhase phase ) override {
            ...
            chapter4::pipeline_cache.insert( hash_calculate( "pbr_lighting" ), pbr_lighting_pipeline );
        }

        void pre_render( FrameGraphRenderContext& context ) override {

            // No acquire necessary as two frames have current/previous final images
            // not acquired from the hdr bloom pass.
            Renderer* renderer = context.renderer;
            GpuDevice* gpu = renderer->gpu;
            if ( gpu->absolute_frame <= 1 ) {
                return;
            }
            ...
            if ( gpu->vulkan_compute_queue_family != gpu->vulkan_main_queue_family ) {
                ...
                ImageHandle hdr_lighting_image = hdr_lighting_resource->resource_info.texture.image;
                cb->acquire_image_ownership( hdr_lighting_image,
                                             range_color_full(),
                                             gpu->vulkan_compute_queue_family, gpu->vulkan_main_queue_family,
        ...
        void render( FrameGraphRenderContext& context ) override {
            ...
            gpu_commands->bind_pipeline( pipeline );
            gpu_commands->bind_descriptor_set( { renderer->gpu->bindless_descriptor_set, descriptor_set }, { chapter4::scene_cb_offset, light_cb_offset } );
    ...
    struct TransparentPass : public FrameGraphRenderPass {
        void declare_frame_graph_node( FrameGraphResourceContext& context ) override {
            ...
            FrameGraphNodeCreation_v2 transparent_node_info {
                .inputs = {
                    {
                        .type = FrameGraphResourceType_Attachment,
            ...
            context.frame_graph->add_node_v2( transparent_node_info );
        }
        ...
        void update_psos( FrameGraphResourceContext& context, PipelineUpdatePhase phase ) override {
            ...
            chapter4::pipeline_cache.insert( hash_calculate( "transparent_no_cull" ), transparent_no_cull_pipeline );
            ...
            chapter4::pipeline_cache.insert( hash_calculate( "transparent_cull" ), transparent_cull_pipeline );
        }
    ...
    struct BloomPass : public FrameGraphRenderPass {
        ...
        void update_psos( FrameGraphResourceContext& context, PipelineUpdatePhase phase ) override {
            ...
            if ( phase == PipelineUpdatePhase::Destroy ) {
                ...
                renderer->destroy_shader_state( bloom_downsample_shader );
                renderer->destroy_shader_state( bloom_upsample_shader );
                renderer->gpu->destroy_pipeline_layout( pipeline_layout );
            ...
            context.frame_graph->cache_render_pass_output( k_name, renderer->gpu, pipeline_creation.render_pass_output, false );

            PipelineHandle bloom_downsample_pipeline = renderer->create_pipeline( shader_reflection, pipeline_creation );
            chapter4::pipeline_cache.insert( hash_calculate( "bloom_downsample" ), bloom_downsample_pipeline );
            ...
            PipelineHandle bloom_upsample_pipeline = renderer->create_pipeline( shader_reflection, pipeline_creation );
            chapter4::pipeline_cache.insert( hash_calculate( "bloom_upsample" ), bloom_upsample_pipeline );
        }
        ...
        void post_render( FrameGraphRenderContext& context ) override {
            ...
            FlatHashMapIterator it = pipeline_cache.find( hash_calculate( "bloom_downsample" ) );
            RASSERT( it.is_valid() );
            ...
            // Downsample
            PipelineHandle downsample_pipeline = pipeline_cache.get( it );
            ...
            // Upsample
            it = pipeline_cache.find( hash_calculate( "bloom_upsample" ) );
            RASSERT( it.is_valid() );
            PipelineHandle upsample_pipeline = pipeline_cache.get( it );

            gpu_commands->bind_pipeline( upsample_pipeline );
        ...
        ShaderStateHandle           bloom_downsample_shader;
        ShaderStateHandle           bloom_upsample_shader;
...
int main( int argc, char** argv ) {
    ...
    // window
    WindowConfiguration wconf{ 1280, 800, "Chapter 4: Async", &MemoryService::instance()->system_allocator};
    ...
    chapter4::pipeline_cache.init( allocator, 16 );
    ...
    // Load frame graph
    {
        ...
        frame_renderer.add_render_pass( "depth_prepass", rnewa( chapter4::DepthPrePass, allocator, 64 ) );
        frame_renderer.add_render_pass( "gbuffer_pass", rnewa( chapter4::GBufferPass, allocator, 64 ) );
        frame_renderer.add_render_pass( "lighting_pass", rnewa( chapter4::LighPass, allocator, 64 ) );
        frame_renderer.add_render_pass( "transparent_pass", rnewa( chapter4::TransparentPass, allocator, 64 ) );
        frame_renderer.add_render_pass( "hdr_color_copy_pass", rnewa( chapter4::HDRColorCopyPass, allocator, 64 ) );
        frame_renderer.add_render_pass( "bloom_pass", rnewa( chapter4::BloomPass, allocator, 64 ) );
    ...
    for ( i32 arg_i = 1; arg_i < argc; ++arg_i ) {
        ...
        render_scene.add_and_load_model( file_name, file_base_path, &scratch_allocator );
    }

    // Create scene resources
    {
        // Create scene mesh instances buffer with all material data
        chapter4::scene_mesh_instances_ssbo = gpu.create_buffer( {
            .size = sizeof( chapter4::MeshData ) * render_scene.mesh_instances.size,
        ...
        FlatHashMapIterator it = chapter4::pipeline_cache.find( hash_calculate( "gbuffer_no_cull" ) );
        RASSERT( it.is_valid() );

        PipelineHandle gbuffer_pipeline = chapter4::pipeline_cache.get( it );
        ...
        ds_creation.ssbos = {
            {.buffer = chapter4::scene_mesh_instances_ssbo,
             .binding = renderer.get_binding_index( reflection_info, "MeshData" ) },
        };
        ds_creation.dynamic_buffers = {
            {.binding = renderer.get_binding_index( reflection_info, "LocalConstants" ), .size = sizeof( chapter4::GpuSceneData ) },
        };

        chapter4::scene_ds = gpu.create_descriptor_set( ds_creation );
    }
    ...
    while ( !window.requested_exit ) {
...

        {
            ...
            chapter4::GpuSceneData* uniform_data = gpu.dynamic_buffer_allocate<chapter4::GpuSceneData>( &chapter4::scene_cb_offset );
            if ( uniform_data ) {
            ...
            if ( update_mesh_data )
            {
                ...
                Buffer* mesh_buffer = gpu.get_buffer( chapter4::scene_mesh_instances_ssbo );
                RASSERT( mesh_buffer );
                chapter4::MeshData* mesh_data = ( chapter4::MeshData* )mesh_buffer->mapped_data;
                if ( mesh_data ) {
                    ...
                    gpu.flush_buffer( chapter4::scene_mesh_instances_ssbo, 0, sizeof( chapter4::MeshData ) * render_scene.mesh_instances.size );
                }
            }
    ...
    for ( FlatHashMapIterator it = chapter4::pipeline_cache.iterator_begin(); it.is_valid();
          chapter4::pipeline_cache.iterator_advance( it ) ) {
        PipelineHandle pipeline = chapter4::pipeline_cache.get( it );
        gpu.destroy_pipeline( pipeline );
    }
    chapter4::pipeline_cache.shutdown();

    gpu.destroy_descriptor_set( chapter4::scene_ds );
    gpu.destroy_buffer( chapter4::scene_mesh_instances_ssbo );
