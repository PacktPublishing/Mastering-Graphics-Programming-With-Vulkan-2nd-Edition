namespace optimizer_step_texture {
namespace set_1 {
...
static raptor::DescriptorSetLayoutHandle create_descriptor_set_layout( raptor::GpuDevice* gpu ) {
	...
	dsl = gpu->create_descriptor_set_layout( {
		...
		.set_index = 1,
		.name="optimizer_step_texture_dsl" });
	return dsl;
}

static raptor::DynamicBufferBinding dynamic_buffers_dsc[] = {  { 0, 0 }, };

static void dynamic_buffers(  u32 g_optimizer_texture_constants_size) {
	dynamic_buffers_dsc[ 0 ].size = g_optimizer_texture_constants_size;
}
