namespace train_brdf_with_latent_texture {
namespace set_1 {
...
static raptor::DescriptorSetLayoutHandle create_descriptor_set_layout( raptor::GpuDevice* gpu ) {
	...
	dsl = gpu->create_descriptor_set_layout( {
		...
		.set_index = 1,
		.name="compute_train_brdf_with_latent_texture_dsl" });
	return dsl;
}

static raptor::DynamicBufferBinding dynamic_buffers_dsc[] = {  { 0, 0 }, { 1, 0 } };

static void dynamic_buffers(  u32 g_random_batch_constants_size, u32 g_brdf_latent_training_constants_size) {
	dynamic_buffers_dsc[ 0 ].size = g_random_batch_constants_size;
	dynamic_buffers_dsc[ 1 ].size = g_brdf_latent_training_constants_size;
}
...
static raptor::DescriptorSetHandle create_descriptor_set( raptor::GpuDevice* gpu ) {
	return gpu->create_descriptor_set( {
		...
		.layout = dsl,
		.name = "compute_train_brdf_with_latent_texture_ds"} );
}
} // namespace set_1
} // train_brdf_with_latent_texture
