namespace train_sampler_with_latent_texture {
namespace set_1 {
...
static idra::DescriptorSetLayoutHandle create_descriptor_set_layout( idra::GpuDevice* gpu ) {
	...
	dsl = gpu->create_descriptor_set_layout( {
		...
		.debug_name="train_sampler_with_latent_texture_dsl" });
	return dsl;
}

static idra::DynamicBufferBinding dynamic_buffers_dsc[] = {  { 0, 0 }, };

static void dynamic_buffers(  u32 g_sampler_latent_training_constants_size) {
	dynamic_buffers_dsc[ 0 ].size = g_sampler_latent_training_constants_size;
}
...
static idra::DescriptorSetHandle create_descriptor_set( idra::GpuDevice* gpu ) {
	return gpu->create_descriptor_set( {
		...
		.layout = dsl,
		.debug_name = "train_sampler_with_latent_texture_ds"} );
}
} // namespace set_1
} // train_sampler_with_latent_texture
