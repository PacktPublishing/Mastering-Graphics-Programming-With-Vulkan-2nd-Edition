...
void parse_binary( const u32* data, size_t data_size, StringBuffer& temp_name_buffer,
    ...
    while ( word_index < spv_word_count ) {
        ...
        switch( op ) {
            ...
            case ( SpvOpName ):
            {
                ...
                char* name_view = temp_name_buffer.append_use( name );

                id.name.data = name_view;
                id.name.size = strlen( name_view );
            ...
            case ( SpvOpMemberName ):
            {
               RASSERT( word_count >= 4 );
                ...
                member.name.data = name_view;
                member.name.size = strlen( name_view );
        ...
        if ( id.op == SpvOpVariable ) {
            switch ( id.storage_class ) {
                case SpvStorageClassStorageBuffer:
                {
                    ...
                    DescriptorSetLayoutInfo& setLayout = parse_result->sets[ id.set ];
                    setLayout.set_set_index( id.set );

                    DescriptorSetLayoutInfo::Binding binding{ };
                    ...
                    switch ( uniform_type.op ) {
                        case (SpvOpTypeStruct):
                        {
                            ...
                            binding.name = uniform_type.name.data;

                            // If binding is using a struct, uniform_type will contain
                            // the type and the proper binding name is the id.name.
                            //
                            // Example:
                            // struct Constants {};
                            // StructuredBuffer<Constants> : ...
                            if ( strlen( id.name.data ) > 0 ) {
                                binding.name = id.name.data;
                            }
                        ...
                        default:
                        {
                            rprint( "Error reading op %u %s\n", uniform_type.op, uniform_type.name.data );
                            break;
                        }
                    }

                    // Store binding name
                    binding.name = name_buffer.append_use( binding.name );
                ...
                {
                    // Skip global textures, images and samplers
                    if ( id.set == k_bindless_set_index && ( id.binding == k_bindless_texture_binding || id.binding == ( k_bindless_texture_binding + 1 ) || id.binding == ( k_bindless_texture_binding + 2) ) ) {
                    ...
                    switch ( uniform_type.op ) {
                        ...
                        case SpvOpTypeRuntimeArray:
                        {
                            // NOTE(marco): ignore for now, it's likely a bindless resource
                            break;
                        }
