...
void StringView::copy_to( const StringView& a, char* buffer, sizet buffer_size ) {
    ...
    const sizet max_length = buffer_size - 1 < a.size ? buffer_size - 1 : a.size;
    memory_copy( buffer, (void*)a.data, max_length );
    buffer[ a.size ] = 0;
}
...
// StringArray ////////////////////////////////////////////////////////////
void StringArray::init( u32 buffer_size_, u32 initial_string_count, Allocator* allocator_ ) {

    allocator = allocator_;
    buffer_size = buffer_size_;
    current_size = 0;

    // Calculate the size of the total memory allocation including alignments
    sizet offset = 0;
    offset = memory_align( offset, alignof( FlatHashMap<u64, u32> ) );
    const sizet hash_map_offset = offset;
    offset += sizeof( FlatHashMap<u64, u32> );

    offset = memory_align( offset, alignof( FlatHashMapIterator ) );
    const sizet iterator_offset = offset;
    offset += sizeof( FlatHashMapIterator );

    // Data is char, so aligns to 1 byte
    const sizet data_offset = offset;
    offset += buffer_size_;

    // Offset now contains the final size of the allocation
    const sizet total_allocation_size = offset;

    // Allocate also memory for the hash map
    char* allocated_memory = ( char* )allocator_->allocate( total_allocation_size, 1 );
    string_to_index = ( FlatHashMap<u64, u32>* )( allocated_memory + hash_map_offset );
    string_to_index->init( allocator, initial_string_count );
    string_to_index->set_default_value( u32_max );

    strings_iterator = ( FlatHashMapIterator* )( allocated_memory + iterator_offset );

    data = allocated_memory + data_offset;
}
...
cstring StringArray::intern( cstring string ) {
    ...
    RASSERT( current_size + (u32)length + 1 < buffer_size );
