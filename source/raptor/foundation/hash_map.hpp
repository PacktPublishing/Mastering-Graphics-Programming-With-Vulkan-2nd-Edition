...
namespace raptor {
    ...
    struct FlatHashMap {
        ...
        KeyValue                    default_key_value = { (K)-1, { } };
    ...
    // Capacity ///////////////////////////////////////////////////////////
    static constexpr u64 k_min_capacity         = GroupSse2Impl::kWidth - 1;    // 15
    static constexpr u64 k_group_width          = GroupSse2Impl::kWidth;        // 16
    static constexpr u64 k_cloned_ctrl_bytes     = k_group_width - 1;            // 15

    // ctrl has:
    // - [0 .. capacity-1] normal ctrl bytes
    // - [capacity] sentinel
    // - [capacity+1 .. capacity+kClonedCtrlBytes] clones of [0 .. kClonedCtrlBytes-1]
    // - plus 1 extra byte to make loads of kGroupWidth safe (capacity + 1 + kGroupWidth total)
    static constexpr u64 k_sentinel_bytes = 1;
    static constexpr u64 k_ctrl_extra_bytes = k_sentinel_bytes + k_group_width; // 1 + 16
    ...
    // - For (capacity+1) < Group::kWidth, growth == k_min_capacity. In this case, we
    ...
    static void ConvertDeletedToEmptyAndFullToDeleted( i8* ctrl, size_t capacity ) {
        ...
        RASSERTM( capacity_is_valid( capacity ), "Invalid capacity" );

        // We must cover ctrl[0..capacity] inclusive (sentinel included) and we can
        // safely do SIMD loads/stores because the allocation includes extra bytes.
        for ( i8* pos = ctrl; pos != ctrl + capacity + 1; pos += GroupSse2Impl::kWidth ) {
            GroupSse2Impl{ pos }.ConvertSpecialToEmptyAndFullToDeleted( pos );
        }
        
        // Rebuild cloned bytes (mirror of the first kWidth-1 ctrl bytes)
        raptor::memory_copy( ctrl + capacity + k_sentinel_bytes, ctrl, k_cloned_ctrl_bytes );

        // Restore sentinel explicitly (the conversion step overwrote it)
    ...
    void FlatHashMap<K,V>::reset_ctrl() {
        memset( control_bytes, k_control_bitmask_empty, capacity + k_ctrl_extra_bytes );
        control_bytes[ capacity ] = k_control_bitmask_sentinel;
        raptor::memory_copy( control_bytes + capacity + k_sentinel_bytes, control_bytes, k_cloned_ctrl_bytes );
    ...
    inline void FlatHashMap<K, V>::init( Allocator* allocator_, u64 initial_capacity ) {
        ...
        default_key_value = { ( K )-1, { } };

        control_bytes = group_init_empty();
        slots_ = nullptr;
        reserve( initial_capacity < k_min_capacity ? k_min_capacity : initial_capacity );
    }

    template<typename K, typename V>
    inline void FlatHashMap<K, V>::shutdown() {
        if ( control_bytes != group_init_empty() ) {
            rfree( control_bytes, allocator );
        }
        control_bytes = group_init_empty();
        slots_ = nullptr;
        capacity = size = growth_left = 0;
    }
    ...
    FlatHashMapIterator FlatHashMap<K, V>::find( const K& key ) {
        ...
        while ( true ) {
            ...
            sequence.next();
            RASSERTM( sequence.get_index() <= capacity, "Full table!" );
        }
    ...
    void FlatHashMap<K, V>::erase_meta( const FlatHashMapIterator& iterator ) {
        ...
        // Calculate zeros only if bitmasks are valid - __buildint_cts(0) is undefined.
        bool was_never_full = empty_before && empty_after &&
            ( empty_after.TrailingZeros() + empty_before.LeadingZeros() ) < GroupSse2Impl::kWidth;
    ...
    void FlatHashMap<K, V>::drop_deletes_without_resize() {
        ...
        // capacity is a mask (2^n - 1), so the number of slots is capacity + 1.
    ...
    u64 FlatHashMap<K, V>::calculate_size( u64 new_capacity ) {
        const u64 ctrl_bytes = new_capacity + k_ctrl_extra_bytes;
        const u64 slot_count = new_capacity + 1;
        return ctrl_bytes + slot_count * sizeof( KeyValue );
    }
    ...
    void FlatHashMap<K, V>::initialize_slots() {
        ...
        slots_ = reinterpret_cast< KeyValue* >( new_memory + capacity + k_ctrl_extra_bytes );
    ...
    void FlatHashMap<K, V>::resize( u64 new_capacity ) {
        ...
        RASSERTM( capacity_is_valid( new_capacity ), "Invalid capacity" );
        ...
        size_t total_probe_length = 0;
        // +1 because of the sentinel control byte
    ...
    void FlatHashMap<K, V>::set_ctrl( u64 i, i8 h ) {
        ...
        control_bytes[ i ] = h;
        control_bytes[ ( ( i - GroupSse2Impl::kWidth ) & capacity ) + GroupSse2Impl::kWidth ] = h;
    }
    ...
    inline void FlatHashMap<K, V>::iterator_skip_empty_or_deleted( FlatHashMapIterator& it ) {
        ...
        RASSERTM( it.index <= capacity, "Iterator out of range" );
        ...
        RASSERTM( it.index == k_iterator_end || it.index <= capacity, "Iterator out of range" );
    }
    ...
    bool capacity_is_valid( size_t n ) { 
        return ( ( n + 1 ) & n ) == 0 && ( n >= k_min_capacity ); 
    }

    // Rounds up the capacity to the next power of 2 minus 1, with a minimum of k_min_capacity (15).
    u64 capacity_normalize( u64 n ) { 
        //return n ? ~u64{} >> lzcnt_soft( n ) : 1;
        return ( n < k_min_capacity ) ? ( k_min_capacity ) : ( ~u64{} >> lzcnt_soft( n ) );
    }
