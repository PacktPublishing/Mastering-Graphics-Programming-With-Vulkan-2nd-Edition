...
sizet file_write( void* memory, u32 element_size, u32 count, FileHandle file ) {
    return fwrite( memory, element_size, count, file );
}

sizet file_read( void* memory, u32 element_size, u32 count, FileHandle file ) {
    return fread( memory, element_size, count, file );
}
...
bool file_copy( cstring existing_file, cstring new_file ) {
#if defined(_WIN64)
    return CopyFileA( existing_file, new_file, 0 );
#else
    int source = open( existing_file, O_RDONLY, 0 );
    int dest = open( new_file, O_WRONLY | O_CREAT /*| O_TRUNC/**/, 0644 );
...
void file_write_binary( cstring filename, const void* memory, sizet size ) {
    FILE* file = fopen( filename, "wb" );
    fwrite( memory, size , 1, file );
    fclose( file );
}

void file_write_text( FileHandle handle, cstring text ) {
    sizet write_len = strlen( text );
    sizet written = fwrite( text, 1, write_len, handle );

    RASSERT( write_len == written );
}
