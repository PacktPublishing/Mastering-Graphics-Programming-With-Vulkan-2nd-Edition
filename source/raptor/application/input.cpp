...
static bool init_gamepad( int32_t index, Gamepad& gamepad ) {
    ...
    SDL_Gamepad* pad = SDL_OpenGamepad( index );
    ...
    if ( pad ) {
        ...
        gamepad.index = index;
        gamepad.name = SDL_GetJoystickName( joy );
        gamepad.handle = pad;
        gamepad.id = SDL_GetJoystickID( joy );
...
void InputBackend::get_mouse_state( InputVector2& position, u8* buttons, u32 num_buttons ) {
    ...
    for ( u32 i = 0; i < num_buttons; ++i ) {
        ...
        buttons[ i ] = mouse_buttons & SDL_BUTTON_MASK( sdl_button );
    }
...
void InputBackend::on_event( void* event_, u8* keys, u32 num_keys, Gamepad* gamepads, u32 num_gamepads, bool& has_focus ) {
    ...
    switch ( event.type ) {
        ...
        {
            i32 key = event.key.scancode;
            if ( key >= 0 && key < (i32)num_keys )
                keys[ key ] = ( event.type == SDL_EVENT_KEY_DOWN );
        ...
        case SDL_EVENT_GAMEPAD_AXIS_MOTION:
        {
            ...
            for ( size_t i = 0; i < k_max_gamepads; i++ ) {
                if ( gamepads[ i ].id == event.gaxis.which ) {
                    gamepads[ i ].axis[ event.gaxis.axis ] = event.gaxis.value / 32768.0f;
                    break;
                }
            }
            break;
        ...
        case SDL_EVENT_GAMEPAD_BUTTON_UP:
        {
            ...
            for ( size_t i = 0; i < k_max_gamepads; i++ ) {
                if ( gamepads[ i ].id == event.gbutton.which ) {
                    gamepads[ i ].buttons[ event.gbutton.button ] = event.gbutton.down;
                    break;
                }
            }
            break;
