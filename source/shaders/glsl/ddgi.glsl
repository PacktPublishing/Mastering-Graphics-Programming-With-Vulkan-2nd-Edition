...
void main() {
    ...
    if (gl_HitKindEXT == gl_HitKindBackFacingTriangleEXT) {
        ...
        if ( use_infinite_bounces() ) {
            diffuse += albedo * sample_irradiance( world_position, normal, frame.camera_position.xyz ) * infinite_bounces_multiplier;
        }
...
void main() {
    ...
    vec2 screen_uv = uv_nearest(coords.xy, frame.resolution / resolution_divider);
    ...
    vec3 irradiance = sample_irradiance( pixel_world_position, normal, frame.camera_position.xyz );
...
void main() {
    ...
    gl_Position = frame.view_projection * vec4( (position * probe_sphere_scale) + probe_position, 1.0 );
    ...
    normal_edge_factor.w = abs(dot(normal_edge_factor.xyz, normalize(probe_position - frame.camera_position.xyz)));
}
