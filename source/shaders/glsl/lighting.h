...
vec3 calculate_raytraced_directional_light_contribution(vec4 albedo, float roughness, vec3 normal, vec3 emissive, vec3 world_position, vec3 v, vec3 F0, float NoV, uvec2 pixel_position, vec3 light_direction) {
...
vec3 calculate_point_light_contribution(vec4 albedo, float roughness, vec3 normal, vec3 emissive,
                                        ...
                                        uvec2 pixel_position, uint shadow_light_index, float metallic,
    ...
    if ( attenuation > 0.0001f && NoL > 0.0001f ) {
        ...
        const float D = d_ggx( roughness, NoH, h );
        const float V = v_smith_ggx_correlated_fast( roughness, NoV, NoL );
...
vec3 apply_volumetric_fog( vec2 screen_uv, float raw_depth, vec3 color ) {
    ...
    // Dither opacity (not T), and clamp.
    float noise_modifier = triangular_noise( noise.x, noise.y ) * frame.volumetric_fog_application_dithering_scale;
    opacity = clamp( opacity + noise_modifier, 0.0f, 1.0f );
    T = 1.0f - opacity;

    vec3 fog_scattering = st.rgb;

    // If integration stored scattering predivided by opacity, undo it here.
    if ( enable_volumetric_fog_opacity_anti_aliasing() ) {
        fog_scattering *= max( opacity, 1e-6f );
    }

    color.rgb = color.rgb * T + fog_scattering;
#endif // ENABLE_VOLUMETRIC_FOG
...
vec4 calculate_lighting(vec4 base_colour, vec3 orm, vec3 normal, vec3 emissive, vec3 world_position, uvec2 pixel_position, vec2 screen_uv, bool transparent) {
    ...
    vec3 F0 = mix(vec3(0.04), base_colour.rgb, metallic);
    ...
    uint address = get_tile_address( pixel_position );
    ...
    // We maintain the naming "word" to be closer to the original paper,
    // but in reality it is just an alias to say uint32.
    uint word_min = max( merged_min / 32, 0 );
    uint word_max = min( merged_max / 32, LIGHT_MASK_U32_COUNT );
    ...
    for ( uint word_index = word_min; word_index <= word_max; ++word_index ) {
        ...
        while ( merged_mask != 0) {
            ...
            final_color.rgb += calculate_point_light_contribution( albedo, roughness, normal, emissive, world_position,
                                                                   V, F0, NoV, pixel_position, global_light_index, metallic, perceptual_roughness );
        }
    }
#else
    ...
    if (!transparent) {
        ...
        if ( is_raytrace_shadow_point_light() ) {
            final_color.rgb += calculate_raytraced_point_light_contribution( albedo, roughness, normal, emissive, world_position, V, F0, NoV, pixel_position );
        }
        else {
            // Directional light
    ...
    return vec4( final_color.rgb, final_color.a );
}
