...
layout ( std430, set = MATERIAL_SET, binding = 13 ) readonly buffer MeshAABBs {

    vec4        aabbs[];    // 2 vec4 for each aabb
} mesh_aabb_sb;
...
bool has_skinning( uint flags ) {
    return (flags & DrawFlags_HasJoints) != 0 && (flags & DrawFlags_HasWeights) != 0;
}

#if defined (FRAGMENT)

// Apply both alpha mask and/or dithering based on flags
void apply_alpha_discards( uint flags, float alpha, float alpha_cutoff, vec2 uv, uint alpha_texture_index ) {
    ...
    if ( alpha_texture_index != INVALID_TEXTURE_INDEX ) {
        float alpha = texture(global_textures[ nonuniformEXT( alpha_texture_index ) ], uv).r;
