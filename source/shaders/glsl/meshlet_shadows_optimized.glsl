...
// Per mesh instance in the tile: first global meshlet (needed by the balanced expansion).
shared uint s_meshlet_offset[256];

// Wave totals per face, then their exclusive scan. 32 slots = 256 lanes / subgroup size >= 8.
const uint k_max_waves = 32;
shared uint s_wave_total[6 * k_max_waves];

// Local (tile-relative) base of every face in the allocation, and the global allocation base.
shared uint s_face_local_base[6];
shared uint s_allocation_base;
...
void main() {
    ...
    if (mesh_instance_index < pc.mesh_instance_count) {
        ...
        if ((mesh_draw.flags & (DrawFlags_AlphaMask | DrawFlags_Transparent)) == 0) {
            ...
            if (aabb_intersects_sphere(world_aabb_min, world_aabb_max, light.world_position, light.radius)) {
                ...
                meshlet_offset = mesh_draw.meshlet_offset;
                face_mask = calculate_face_mask(light.world_position, light.radius, world_center, world_radius);
            }
        }
    }

    s_meshlet_offset[lane] = meshlet_offset;

    // Phase 2: six exclusive scans in parallel (subgroup scan + one level over the waves).
    uint lane_prefix[6];
    for (uint face = 0; face < 6; ++face) {
        const uint contribution = ((face_mask >> face) & 1u) != 0u ? meshlet_count : 0u;
        lane_prefix[face] = subgroupExclusiveAdd(contribution);
        const uint wave_total = subgroupAdd(contribution);
        if (subgroupElect()) {
            s_wave_total[face * k_max_waves + gl_SubgroupID] = wave_total;
        }
    }

    barrier();

    // Six lanes scan the (few) wave totals of their face.
    if (lane < 6) {
        uint running = 0;
        for (uint wave = 0; wave < gl_NumSubgroups; ++wave) {
            const uint total = s_wave_total[lane * k_max_waves + wave];
            s_wave_total[lane * k_max_waves + wave] = running;
            running += total;
        }
        s_face_total[lane] = running;
    }

    barrier();

    for (uint face = 0; face < 6; ++face) {
        s_face_prefix[face * 256 + lane] = s_wave_total[face * k_max_waves + gl_SubgroupID] + lane_prefix[face];
    }

    if (lane == 0) {
        uint total_meshlet_count = 0;
        for (uint face = 0; face < 6; ++face) {
            s_face_local_base[face] = total_meshlet_count;
            total_meshlet_count += s_face_total[face];
        }
        ...
        if (total_meshlet_count > 0) {
            ...
            s_allocation_base = allocation_base;

            for (uint face = 0; face < 6; ++face) {
                s_face_base[face] = allocation_base + s_face_local_base[face];
            }
    ...
    // Phase 3: balanced expansion. Every lane writes total/256 entries, whatever mesh they belong to.
    // Same layout as before: entry k goes to allocation_base + k.
    const uint tile_first_instance = tile_index * 256;

    for (uint k = lane; k < s_total_meshlet_count; k += 256) {
        // Last face whose base is <= k (empty faces share the base of the next one).
        uint face = 0;
        for (uint f = 1; f < 6; ++f) {
            if (s_face_local_base[f] <= k) {
                face = f;
            }
        }

        const uint k_in_face = k - s_face_local_base[face];

        // Last instance lane whose prefix is <= k_in_face: upper_bound - 1 (8 steps).
        uint lo = 0;
        uint hi = 256;
        while (lo < hi) {
            const uint mid = (lo + hi) >> 1;
            if (s_face_prefix[face * 256 + mid] <= k_in_face) {
                lo = mid + 1;
            } else {
                hi = mid;
            }
        ...
        const uint source_lane = lo - 1;
        const uint meshlet_in_mesh = k_in_face - s_face_prefix[face * 256 + source_lane];

        ShadowMeshletInstance candidate;
        candidate.mesh_instance_index = tile_first_instance + source_lane;
        candidate.global_meshlet_index = pack_shadow_meshlet(s_meshlet_offset[source_lane] + meshlet_in_mesh, 1u << face);

        shadow_meshlet_instances_sb.instances[s_allocation_base + k] = candidate;
    }
