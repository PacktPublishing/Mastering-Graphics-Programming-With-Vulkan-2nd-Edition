...
#define LIGHT_MASK_U32_COUNT ( ( NUM_LIGHTS + 31 ) / 32 )
...
layout ( set = GLOBAL_SET, binding = BINDLESS_BINDING ) uniform sampler2DArray global_texture_arrays[];
...
layout( set = GLOBAL_SET, binding = BINDLESS_IMAGES ) writeonly uniform image2DArray global_image_arrays_2d[];
...
vec2 uv_from_compute_dispatch( uvec2 global_invocation_id, vec2 rcp_size ) {
    return vec2( (global_invocation_id + vec2(0.5f) ) * rcp_size );
}

bool depth_is_closer( float a, float b ) {
...
// Convert linear depth to slice 0..1
float linear_depth_to_slice_f(float near, float far, float linear_depth, int num_slices) {
    linear_depth = clamp(linear_depth, near, far);

    // This matches (slice + 0.5)/N when slice_to_exponential_depth uses +0.5
    float u = log2(linear_depth / near) / log2(far / near);

    // Clamp to texel centers.
    float inv_slices = 1.0 / float(num_slices);
    return clamp(u, 0.5 * inv_slices, 1.0 - 0.5 * inv_slices);
}
...
vec2 animated_blue_noise_golden_ratio(uvec2 pixel, uint frame, uint blue_noise_texture_index) {
    // Blue noise texture size is 128x128
    pixel = (pixel % 128);
    // Read blue noise from texture
    vec2 blue_noise = texelFetch(global_textures[nonuniformEXT(blue_noise_texture_index)], ivec2(pixel), 0).rg;
    const float k_golden_ratio_conjugate = 0.61803398875;

    frame = (frame % 64);
    float blue_noise0 = fract((blue_noise.r) + float(frame) * k_golden_ratio_conjugate);
    float blue_noise1 = fract((blue_noise.g) + float(frame) * k_golden_ratio_conjugate);
...
vec2 animated_blue_noise(uvec2 pixel, uint frame, uint blue_noise_texture_index) {
    ...
    // Add different scrambling for the channels 
    uvec2 scramble = uvec2(frame * 17u, frame * 29u);
    // Use bit masking instead of modulo as it is cheaper (size 128x128)
    uvec2 p = (pixel + scramble) & 127u;
    // Read texture without filtering 
    vec2 bn = texelFetch(global_textures[nonuniformEXT(blue_noise_texture_index)],
                         ivec2(p), 0).rg;

    // Add two distinct steps to avoid correlating channels over time
    const float k0 = 0.61803398875;
    const float k1 = 0.75487766625;

    float f = float(frame);
    return vec2(fract(bn.r + f * k0),
                fract(bn.g + f * k1));
}
