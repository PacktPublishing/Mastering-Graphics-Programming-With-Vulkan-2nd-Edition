...
void main() {
    ...
    uint meshlet_connectivity_data_offset = meshlets[global_meshlet_index].connectivity_data_offset;
    ...
    if (gl_LocalInvocationID.x == 0) {
        ...
        group_meshlet_data[2] = meshlets[cached_meshlet_index].connectivity_data_offset;
    ...
    uint meshlet_connectivity_data_offset = group_meshlet_data[2];
    uint triangle_count = group_meshlet_data[3];

#endif

    uint index_data_offset = meshlet_connectivity_data_offset + vertex_count;
...
void main() {
    ...
    uint mesh_instance_index = draw_commands[draw_command_index].mesh_instance_index;
    uint mesh_draw_index = mesh_instance_draws[mesh_instance_index].mesh_draw_index;
...
void main() {
    ...
    uint connectivity_data_offset = meshlets[meshlet_index].connectivity_data_offset;
    ...
    gl_Position = frame.view_projection * (model * vec4(position, 1));
...
#if defined(DEBUG)
    vColor = hash_color(meshlet_index);
    triangle_index = vi;
#endif
