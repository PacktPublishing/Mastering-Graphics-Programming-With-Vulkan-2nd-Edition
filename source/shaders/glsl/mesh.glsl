...
void main() {
    ...
    gl_Position = frame.view_projection * mesh_draw.model * vec4(position, 1.0);
...
void main() {
    ...
    apply_alpha_discards( flags, base_colour.a, mesh_draw.alpha_cutoff, vTexcoord0, mesh_draw.alpha_texture_index );
...
void main() {
    ...
    vec4 final_color = calculate_lighting( base_colour, pbr_parameters, normal, emissive_colour, world_position, position, vec2(0,0), true );
    final_color.rgb = encode_srgb( final_color.rgb );
    frag_colorfrag_color = final_color;
}
